...
Connected live via Import mode against the Synapse serverless SQL endpoint — four visuals covering monthly trend, revenue by borough/hour, tip behavior by payment type, and busiest pickup zones.

## Orchestration: ADF Triggering Databricks Directly

Beyond ingestion, ADF orchestrates the full batch chain end-to-end: the ingestion pipeline is directly followed by two **Databricks Notebook activities** (bronze→silver transform, then silver→gold aggregation), chained via success dependencies. A single pipeline run now produces full lineage from raw source to gold tables, visible in one place in ADF's monitoring UI — rather than ingestion and transformation being two separately-triggered, disconnected steps.

**Job cluster vs. existing interactive cluster:** the standard production pattern is to have ADF spin up a dedicated, ephemeral **job cluster** per run (isolated, auto-terminating, no risk of interfering with other work). This project's free-tier subscription has a default regional core quota of 4 — too low to run a new job cluster alongside the existing interactive cluster at the same time. Rather than request a quota increase (not reliably available on free trial subscriptions) or leave the pipeline broken, the Databricks Linked Service was configured to run notebook activities against the **existing interactive cluster** instead, with that cluster started manually before triggering the pipeline. This is a deliberate, quota-driven trade-off — in a production environment with normal quota limits, a dedicated job cluster is the correct choice for isolation and cost-scoped compute.
